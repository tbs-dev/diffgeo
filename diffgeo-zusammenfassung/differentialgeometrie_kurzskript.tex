...
\[ \int_{\partial G} J(F) \diff \mathcal{H}= \int_a^b \ip{F(c(t))}{n_a(c(t))}\norm{\dot{c}(t)}\diff t =\int_a^b f(c(t))\norm{\dot{c}(t)}\diff t=\int_c f \diff s.\] Erinnern wir uns nun daran, dass \[  \int_G (\partial_1 F_1+\partial_2F_2)(x,y)\diff (x,y)=\int_{\partial G}J(F)\diff s \] ist und allgemein die Divergenz eines Vektorfeldes definiert ist durch \[ \div F(x):=\sum_{j=1}^n \partial_jF_j(x).\] Damit ergibt sich folgender wichtiger Satz.
\begin{theo}[Satz von Gauß]
	ei $G\subseteq \R^2$ ein zulässiges Gebiet, d.h. $\partial G$ ist Bild einer regulären geschlossenen stückweisen $C^1$-Jordankurve und $F$ ein $C^1$-Vektorfeld in einem Gebiet $G'\subseteq \R^2$ mit $G\subseteq G'$. Dann ist \[  \int_G \div F \diff \mathcal{L}^2(x,y)=\int_{\partial G} \ip{F}{n_a} \diff s. \]
\end{theo}
